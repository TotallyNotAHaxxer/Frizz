				...
				case "Payload":
					if PortService[L2_c.DstPort.String()] == "FTP" {
						if APPLET != nil {
							...
							for i := range Use_c {
								...
								Frizz_Data.DatabaseVariable.Creds.Ftp.FTP_Username = Frizz_Helpers.ValueRemover(Frizz_Data.DatabaseVariable.Creds.Ftp.FTP_Username)
								Frizz_Data.DatabaseVariable.CredentialsFTP += 1
							}
							for i := range Use_c0 {
								...
								Frizz_Data.DatabaseVariable.Creds.Ftp.FTP_Password = Frizz_Helpers.ValueRemover(Frizz_Data.DatabaseVariable.Creds.Ftp.FTP_Password)
								Frizz_Data.DatabaseVariable.CredentialsFTP += 1
							}
						}
					}
					...
					if PortService[L2_c.DstPort.String()] == "SMTP" {
						if APPLET != nil {
							...
							if Use_c1 != nil {
								decvaltoswitch := strings.Trim(string(APPLET.Payload()), "AUTH")
								decvaltoswitch = strings.TrimSpace(decvaltoswitch)
								decvaltoswitch = strings.Trim(decvaltoswitch, "PLAIN")
								Decoded_Value := Frizz_Helpers.DECB64(strings.TrimSpace(decvaltoswitch))
								Frizz_Data.DatabaseVariable.Creds.Smtp.SMTP_Plainauth = append(Frizz_Data.DatabaseVariable.Creds.Smtp.SMTP_Plainauth, decvaltoswitch)
								...
								Frizz_Data.DatabaseVariable.Creds.Smtp.SMTP_Plainauth_Decodec = Frizz_Helpers.ValueRemover(Frizz_Data.DatabaseVariable.Creds.Smtp.SMTP_Plainauth_Decodec)
								Frizz_Data.DatabaseVariable.Creds.Smtp.SMTP_Plainauth = Frizz_Helpers.ValueRemover(Frizz_Data.DatabaseVariable.Creds.Smtp.SMTP_Plainauth)
								Frizz_Data.DatabaseVariable.CredentialsSMTP += 1
							}
							if Use_c2 != nil {
								...
								if NextPacket == "Finished here!" {
									...
									Frizz_Data.DatabaseVariable.Creds.Smtp_Cram.Decoded = Frizz_Helpers.ValueRemover(Frizz_Data.DatabaseVariable.Creds.Smtp_Cram.Decoded)
									Frizz_Data.DatabaseVariable.Creds.Smtp_Cram.Encoded = Frizz_Helpers.ValueRemover(Frizz_Data.DatabaseVariable.Creds.Smtp_Cram.Encoded)
									Frizz_Data.DatabaseVariable.CredentialsSMTP += 1
								}
							}
						}
					...
					if PortService[L2_c.DstPort.String()] == "SSH" {
						if APPLET != nil {
							Use_c := Use_USER_FTP.FindAllStringSubmatch(string(APPLET.Payload()), 1)
							Use_c0 := Use_PASS_FTP.FindAllStringSubmatch(string(APPLET.Payload()), 1)
							// we reuse the same values because SSH payloads are the same here
							if Use_c != nil {
								Frizz_Data.DatabaseVariable.Creds.Ssh.SSHUsername = Frizz_Helpers.Appender(Use_c, Frizz_Data.DatabaseVariable.Creds.Ssh.SSHUsername)
								Frizz_Data.DatabaseVariable.Creds.Ssh.SSHUsername = Frizz_Helpers.ValueRemover(Frizz_Data.DatabaseVariable.Creds.Ssh.SSHUsername)
							}
							if Use_c0 != nil {
								Frizz_Data.DatabaseVariable.Creds.Ssh.SSHPassword = Frizz_Helpers.Appender(Use_c0, Frizz_Data.DatabaseVariable.Creds.Ssh.SSHPassword)
								Frizz_Data.DatabaseVariable.Creds.Ssh.SSHPassword = Frizz_Helpers.ValueRemover(Frizz_Data.DatabaseVariable.Creds.Ssh.SSHPassword)
							}
						}
					}
				}
