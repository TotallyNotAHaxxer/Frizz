				...
				case "Payload":
					if PortService[L2_c.DstPort.String()] == "FTP" {
						if APPLET != nil {
							...
							for i := range Use_c {
								Cred.Ftp.FTP_Username = append(Cred.Ftp.FTP_Username, strings.Trim(Use_c[i][1], "USER"))
							}
							for i := range Use_c0 {
								Cred.Ftp.FTP_Password = append(Cred.Ftp.FTP_Password, strings.Trim(Use_c0[i][1], "PASS"))
							}
						}
					}
					...
					if PortService[L2_c.DstPort.String()] == "SMTP" {
						if APPLET != nil {
							...
							if Use_c1 != nil {
								...
								Cred.Smtp.SMTP_Plainauth = Frizz_Helpers.Appender(Use_c1, Cred.Smtp.SMTP_Plainauth)
								Cred.Smtp.SMTP_Plainauth_Decodec = append(Cred.Smtp.SMTP_Plainauth_Decodec, Decoded_Value)
							}
							if Use_c2 != nil {
								...
								if NextPacket == "Finished here!" {
									...
									Cred.Smtp_Cram.Decoded = append(Cred.Smtp_Cram.Decoded, Decoded_Value)
									Cred.Smtp_Cram.Encoded = append(Cred.Smtp_Cram.Encoded, string(APPLET.Payload())[0:])
								}
							}
						}
					}
