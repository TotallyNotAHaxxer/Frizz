...
func (Creds *IMAP_Credential) Decoder_IMAP_CREDS(pkt gopacket.Packet) {
					...
					if IMAP_PORTS[lay2_c.DstPort.String()] == "IMAP" {
						...
						if result_plain != nil {
							...
							for i := range result_plain {
								Creds.IMAP_PLAINTEXT = append(Creds.IMAP_PLAINTEXT, strings.Trim(result_plain[i][0], `" LOGIN `))
							}
						}
						...
						if msg == "next" {
							...
							if a {
								...
								Creds.IMAP_BASE64_Decoded = append(Creds.IMAP_BASE64_Decoded, dec)
								Creds.IMAP_BASE64_Decoded = Frizz_Helper.ValueRemover(Creds.IMAP_BASE64_Decoded)
								Creds.IMAP_BASE64_Encoded = append(Creds.IMAP_BASE64_Encoded, string(pkt.ApplicationLayer().Payload()))
								Creds.IMAP_BASE64_Encoded = Frizz_Helper.ValueRemover(Creds.IMAP_BASE64_Encoded)
								msg = "mode"
							}
						}
						...
						*/
						...
						if msg2 == "next" {
							if Frizz_Helper.VALB64(string(pkt.ApplicationLayer().Payload())) {
								dec := Frizz_Helper.DECB64(string(pkt.ApplicationLayer().Payload()))
								Creds.IMAP_DIGEST_MD5_Decoded = append(Creds.IMAP_DIGEST_MD5_Decoded, dec)
								Creds.IMAP_DIGEST_MD5_Encoded = append(Creds.IMAP_DIGEST_MD5_Encoded, string(pkt.ApplicationLayer().Payload()))
								Creds.IMAP_DIGEST_MD5_Encoded = Frizz_Helper.ValueRemover(Creds.IMAP_DIGEST_MD5_Encoded)
								Creds.IMAP_DIGEST_MD5_Decoded = Frizz_Helper.ValueRemover(Creds.IMAP_DIGEST_MD5_Decoded)
							}
							msg2 = "0"
						}
